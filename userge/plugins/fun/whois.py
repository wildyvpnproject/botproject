...
async def who_is(message: Message):
    await message.edit("`Sedang Menyolong Data Target......!`")
    ...
    if from_user or from_chat is not None:
        ...
        message_out_str += f"<b>🏢 ID Target :</b> <code>{from_user.dc_id}</code>\n"
        message_out_str += f"<b>🤖 Bot :</b> <code>{from_user.is_bot}</code>\n"
        message_out_str += f"<b>🚫 Vertifikasi :</b> <code>{from_user.is_scam}</code>\n"
        message_out_str += "<b>✅ Verified Oleh Tele:</b> "
        message_out_str += f"<code>{from_user.is_verified}</code>\n"
        message_out_str += f"<b>🕵️‍♂️ ID User :</b> <code>{from_user.id}</code>\n"
        ...
        try:
            ...
            message_out_str += f"<b>👥 Chats :</b> <code>{cc_no}</code>\n"
        message_out_str += f"<b>📝 Bio :</b> <code>{from_chat.bio}</code>\n\n"
        message_out_str += f"<b>👁 Terakhir Di Lihat :</b> <code>{from_user.status}</code>\n"
        message_out_str += "<b>🔗 Perma Link :</b> "
